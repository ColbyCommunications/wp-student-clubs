...
wp core install \
	--path=$LANDO_WEBROOT \
	--url="http://wp-student-clubs.lndo.site" \
	'--title="Test Site"' \
...
# Link our plugin
ln -sF $LANDO_MOUNT $LANDO_WEBROOT/wp-content/plugins/wp-student-clubs
wp plugin activate wp-student-clubs --path=$LANDO_WEBROOT
